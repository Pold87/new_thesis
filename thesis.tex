...
%  identify well-performing maps.
  ...
  The algorithm has a computational overhead of approximately 15\,ms
  on board of a MAV and could therefore be used for estimation with a
...
of few millimeters. We used OptiTrack as MAV guidance system for
...
obtained by \emph{full sampling} based on $N = 2\,000$ images. The
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.6\columnwidth]{samplescos}
\caption{{\label{fig:cosine} Cosine similarity between histograms
    obtained by random sampling to the histograms obtained by full
    sampling as a function of the number of extracted samples. The
    error bars show the standard deviation of the cosine
    similarity. The squares indicate the positions at which the
...
\caption{{\label{fig:mapexp} The created map that was stitched
    together using 445 images. The ``non-mapped'' area in the center
    of the image is a result of the set flight path. An image
    distortion can be seen at the right-hand side, where the landing
    spot sign appears twice, while in reality, only one circle was
    visible.%
  }}
\end{center}
\end{figure}
...
station. The position estimates were calculated on board of the MAV
using the texton-based approach with the particle filter. The
...
For the triggered landing experiment, the MAV was guided along a set
of random flight paths, which covered the $5 \times 5$ meters area;
during the navigation, the MAV was programmed to land as soon as its
position estimates were in a ``landing zone:'' a $x,y$-position with a
specified radius $r$. A safety criterion was introduced such that the
landing is only performed if the variance of the particles is below a
threshold. The criterion parameter was set to 60\,cm in both $x-$ and
$y$-direction.  The $x,y$-coordinates of circle was specified in the
flight plan; the radius was set to $r = 60\,cm$. We performed six
triggered landings; after each landing, the $x,y$-center of the zone
was randomly set another position in the map.  For the texton
framework, the same training set as in
Experiment~\ref{sec:experiment-real} was used.
...
Adapting the frequency of the main loop of developed approach is one
of its core parts to make it suitable for different platforms with
...
a frequency of 12.5\,Hz. Therefore, the maximum achievable
frequency---without any image processing---is 12.5\,Hz, which is the
baseline for the conducted experiments.

The frequency was determined by varying the following parameters (the
default values were used for the \emph{ceteris paribus} assumption,
when varying the other parameters):
\begin{itemize}
\item number of samples in the texton-based approach
  (Figure~\ref{fig:freqsam}). The frequency using \emph{full sampling}
  was 0.1\,Hz\\default: 400
%\item number of textons
\item number of particles of the particle filter (Figure~\ref{fig:freqpart})\\default: 100
\item number of histograms in the training set (Figure~\ref{fig:freqhist})\\default: 500
\end{itemize}
...
(Experiment~\ref{sec:experiment-real}), the initial mean error was
rather large: 130\,cm in $x$-direction and 90\,cm in $y$-direction. A
more in-depth analysis revealed that the estimates of the particle
filter were lacking behind the position estimates of the motion
tracking system. This is possibly due to the simple motion model of
the particle filter that was based on Gaussian noise only. By shifting
the estimates of the particle filter by six frames, that is
approximately 0.46 seconds at a frequency of 13\,Hz, the error could
be reduced to 46\,cm in $x$-direction and 54\,cm in
$y$-direction. This lag can be addressed by slower speed during flight
or a more sophisticated motion model that includes heading and
velocity.
...
frequency of more than 60\,Hz (15\,ms per iteration) on a single board
computer with limited CPU. Shifting processing power to an offline
training step by creating a training set and relying on random
sampling are the cornerstones of the developed algorithm.
...
To answer RQ\,2, we still need to find evidence that the proposed map
evaluation generalizes to the real-world. In contrast to R\,1, the
...
synthetic images nor the real-world images. Which is an easier
problem.
...
version of such a system can be found in our GitHub
