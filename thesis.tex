...
%  identify well-performing maps.
  ...
  accuracy of approximately 0.6\,m on a $5\times5$ meter area at a
...
predictions. In this experiment, we analyzed we influence of the
...
the error in $x$-direction and $y$-direction, measured as distance in cm
...
estimates were in a ``landing zone'': a $x,y$-position with a
...
randomly set another position in the map. For the texton framework,
...
algorithm using the \emph{default} values is 32\,ms, which includes
the histogram extraction (16\,ms) as well as the $k$-NN predictions,
the filtering and the output of the best $x,y$-coordinate (16\,ms).
...
Discussion, we address the research questions and give directions for
future research.
...
amount of samples suffice to achieve cosine similarities between the
texton histograms larger than $99\,\%$. This sets the stage for large
...
The image with the minimum and the one with the maximum loss value
based are shown in Figure~\ref{fig:minmaximg}. The different patterns
of the images are clearly visible: while the image with the minimum
value roughly fulfills the desired properties---closeby areas have
similar color values, distant areas are dissimilar, the image with the
maximum loss is mainly black resulting in similar histograms all over
and leading to high loss values. The image with the lowest loss value
...
of histograms. To answer RQ\,2, we still have to evaluate, if the
...
investigate if the results of the synthetically generated images
...
different devices, such as handheld cameras.
...
technique with local technique. To this end, odometry estimates using
optical flow or the inclusion or data from the inertial measurement
...
The rotation is done on the basis of data
from the inertial measurement unit (IMU), which is a sensor that
measures the angular displacement of the MAV.
...
a different regression technique is not straightforward: the technique
...
should be able to trade off speed with accuracy. This allows their use
